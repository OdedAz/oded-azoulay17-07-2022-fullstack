...
const CityWeatherCard = ({ cityWeatherInfo }) => {
  ...
  const { favoriteCities, setFavoriteCities, deleteCityFromFavoritsDB } = useFavoriteCitiesFetch();
  const favoritClickHandler = (e) => {
    if (isLiked) {
      setIsLiked(false);
      const newReducedArray = favoriteCities;
      const indexOfCity = favoriteCities.indexOf(
        (city) => city.id === cityWeatherInfo.id
      );
      ...
      setIsLiked(true);
      const isAllreadyInArray = favoriteCities?.some(
        (e) => e.id === cityWeatherInfo.id
      );
      if (!isAllreadyInArray) {
        const newFavoritesArray = [...favoriteCities, cityWeatherInfo];
